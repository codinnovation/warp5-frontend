  ...
  return (
    <div className="absolute top-full left-0 mt-3 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 w-72 max-h-72 overflow-y-auto scroll-smooth z-50 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      {equipmentTypes.map((equipment) => (
        <div
          key={equipment}
          className={`px-4 py-3 text-sm font-medium cursor-pointer rounded-xl transition-all duration-200 flex items-center justify-between group ${selectedEquipment === equipment ? 'bg-green-50 text-green-700' : 'hover:bg-green-50 hover:text-green-700 text-gray-700'
            }`}
          onClick={() => {
            onSelect(equipment);
            onClose();
